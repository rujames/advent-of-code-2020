...
(def card-key 18499292)
(def door-key 8790390)

(defn loop-number [subject key]
  (count (take-while #(not= key %) (reductions (fn [x y] (rem (* x y) 20201227)) 1 (repeat subject)))))

(defn transform [subject loop-number]
  (reduce (fn [x y] (rem (* x y) 20201227)) 1 (take loop-number (repeat subject))))

;; (transform card-key (loop-number 7 door-key))
;; => 18433997
