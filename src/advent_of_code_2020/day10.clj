...
(def joltages (map read-string (str/split (slurp "src/advent_of_code_2020/day10.input") #"\r\n")))
(def chain (concat
            [0]
            (vec (sort joltages))
            [(+ (apply max joltages) 3)]))

(defn n-jolt-differences [n chain]
  (->> chain
       (map - (rest chain))
       (filter #(= n %))
       (count)))

;; (* (n-jolt-differences 1 chain) (n-jolt-differences 3 chain))
;; => 2312
